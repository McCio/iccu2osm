...
complete = complete.with_columns(pl.when(pl.col('stato-registrazione').eq_missing(None)).then(pl.lit('Biblioteca censita')).otherwise('stato-registrazione').alias('stato-registrazione'))
...
contatti = contatti.with_columns(pl.col('valore').str.replace_all('h+tt+ps?[;:]//+\\s*', 'https://'))
...
contatti = contatti.with_columns(pl.when(pl.col('valore').str.contains('facebook', literal=True)).then(pl.lit('facebook'))
...
contatti = contatti.with_columns(pl
  .when(pl.col('valore').str.starts_with('+39') & pl.col('tipo').ne_missing('Fax')).then(pl.lit('Telefono'))
...
contatti = contatti.with_columns(pl.when(pl.col('tipo').ne_missing('Telefono') & pl.col('tipo').ne_missing('Fax')).then('valore').otherwise(pl.col('valore').str.replace_all('[\\. \\(\\)]', '').str.replace('+39', '+39 ', literal=True)).alias('valore'))
...
contatti = contatti.with_columns(pl.when(pl.col('valore').str.contains('@') & pl.col('tipo').ne_missing(pl.lit('PEC'))).then(pl.lit('E-mail')).otherwise('tipo').alias('tipo'))
contatti = contatti.with_columns(pl.when(pl.col('valore').str.contains('@[a-z0-9\\.]*(pec|legalmail)[a-z0-9\\.]*\\.[a-z0-9\\.]+$')).then(pl.lit('PEC')).otherwise('tipo').alias('tipo'))

contatti = contatti.with_columns(pl.when(pl.col('valore').str.starts_with('@') & pl.col('note').str.to_lowercase().str.contains('instagram', literal=True)).then(pl.lit('instagram')).otherwise('tipo').alias('tipo'))
contatti = contatti.with_columns(pl.when(pl.col('valore').str.starts_with('@') & pl.col('note').str.to_lowercase().str.contains('twitter', literal=True)).then(pl.lit('twitter')).otherwise('tipo').alias('tipo'))
contatti = contatti.with_columns(pl.when(pl.col('tipo').ne_missing('twitter')).then('valore').otherwise(pl.col('valore').str.replace('^(@|(https://)?(www\\.)?twitter\\.com/)', '').str.replace('(/|\\?).*$', '')).alias('valore'))
contatti = contatti.filter((pl.col('tipo').eq_missing('instagram') & pl.col('valore').str.contains('/invites/contact', literal=True)).not_())
contatti = contatti.with_columns(pl.when(pl.col('tipo').ne_missing('instagram')).then('valore').otherwise(pl.col('valore').str.replace('^(@|(https://)?(www\\.)?instagram\\.com/)', '').str.replace('(/|\\?).*$', '')).alias('valore'))
contatti = contatti.with_columns(pl.when(pl.col('tipo').ne_missing('facebook')).then('valore').otherwise(pl.col('note').str.replace('^([Pp]agina\\s*|[Gg]ruppo\\s*|[Pp]rofilo\\s*)?[Ff]ace[Bb]+oo?k$', '').str.replace('(/|\\?).*$', '')).alias('note'))
...
contatti_grouped = contatti.group_by("codice-isil", "tipo").agg('valore', 'note').pivot(on='tipo', index='codice-isil',values=("valore", "note"))
...
###### START ADDRESS
# some "indirizzo" end with "Ed 2" or similar, meaning "building #2", that shouldn´t be extracted as housenumber
complete = complete.with_columns(
  pl.col('indirizzo').str.extract('\\s*,?\\s*(s\\.?n\\.?c?|\\d+[/\\a-hA-H-]*)\\s*$', 1).alias('address_housenumber'),
  pl.col('indirizzo').str.replace('\\s*,?\\s*(s\\.?n\\.?c?|\\d+[/\\a-hA-H-]*)\\s*$', '').alias('address_street'),
)
